...
namespace CSOneDriveAccess
{
    public abstract class O365RestSessionBase
    {
        ...
        public string GetLoginUrl(string scopes)
        {
            ...
            string urlStr = $"{AuthRoot}oauth20_authorize.srf?client_id={ClientId}&scope={scopes} offline_access&response_type=token&redirect_uri={CallBackURI}";
        ...
        /// <returns></returns>
        public async Task RedeemTokensAsync(string token)
        {
        ...
        private async Task RefreshTokenIfNeededAsync()
        {
            ...
            //{
            //    string url = $"{AuthRoot}oauth20_token.srf";
            //    byte[] param = Encoding.UTF8.GetBytes($"client_id={ClientId}&redirect_uri={CallBackURI}&client_secret={ClientSecret}&access_token={AccessToken}&grant_type=access_token");
